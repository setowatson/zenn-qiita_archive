---
title: "弊社の人気投稿を機械学習で分析してみる"
emoji: "🌱"
type: "tech" # tech: 技術記事 / idea: アイデア
topics: ["databricks", "機械学習", "データ分析", "zenn", "qiita"]
published: false
...
この記事では、Zennのpublication機能にある記事エクスポート機能を活用し、Databricksのノートブック上で弊社ヘッドウォータースの人気投稿を分析した結果を紹介します。
...
3. 最適なタイトル長: **50 文字**
4. PV獲得トップのユーザー: **hwstakechiyo (合計PV: 73491.00, 記事数: 244)　https://zenn.dev/hwstakechiyo**
5. 最もPVを取りやすくなる記事公開曜日： **水曜日**
6. 公開後のPVピークは公開から： **4 日目**

つまりPV数を稼ぎたい方は、

**「"NVIDIA"の"ハードウェア"に関する記事を"50字前後のタイトル"で"水曜日"に公開すること」**

となります。

タイトル案として、

- 「NVIDIAの最先端ハードウェアが切り拓く未来のテクノロジー」（49字）
- 「GPUからAIチップまで、NVIDIAハードウェアの革新を徹底解説」（51字）
- 「NVIDIAハードウェアの進化とその影響力を最新事例で読み解く」（52字）
- 「NVIDIAが創る次世代ハードウェアの可能性と業界へのインパクト」（50字）
- 「NVIDIA最新GPUの性能とその裏にある革新技術を深掘り解説」（51字）
...
実際に700記事しかなくともPV発生日ごとに列が増えるので、実際のデータとしては61,124行ありました。

![データの構造を示すスクリーンショット](/images/zenn4.png)

タイトルでまとめてみたのですが、なぜか748と合わない…。タイトルを変更している可能性がありますが、一旦スルーします。
...
ちなみに助詞や数字は除かれるようストップワードとして設定しています。
...
みなさん水曜日に投稿しがちなのか、バズってる記事が水曜日に集中しているのか、分かりませんが、おそらく後者な気がします。
...
50字前後のタイトルが良さそうです。
でも45字くらいではPV数が少ないので、そこまで関係ないかもしれません。よく見ると外れ値が引っ張っている様子です。
10〜20字と短いタイトルも良さそうですね。
...
一つ一つ共有すると長くなってしまうので、結論はほぼ上と同じなので、ノートブックの共有のみにします。
...
今回の分析で、人気記事の特徴が定量的に把握できました。これらの知見を活かして、より多くの方に読んでいただける記事を作成していきたいと思います。
...
### Zenn関連
- [Zenn Publication 機能](https://zenn.dev/zenn/articles/what-is-publication)
